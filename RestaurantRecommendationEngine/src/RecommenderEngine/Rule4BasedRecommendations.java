...
public class Rule4BasedRecommendations extends RulesBasedRecommendations{
	
	public Rule4BasedRecommendations(RulesBasedRecommendations nextRuleBasedRecommendation) {
	...
	public void getRestaurantRecommendations(User user, Restaurant[] availableRestaurants) {
		...
		for(int i=0;i<restaurants.size();i++) {
			...
			if(restaurant.getRating()>=4) {
				if((cuisine == cuisineTracking[1].getType() && costBracket == costTracking[0].getType()) ) {
